...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf &kf_state, int &N)
{
  ...
  {
    // 重力对齐：计算旋转使重力指向 Z 轴
    V3D gravity_dir = -mean_acc.normalized();      // IMU 测得的重力方向（向下）
    V3D target_gravity(0, 0, 1);                   // 期望重力方向（Z向上）
    
    Eigen::Quaterniond q_align;
    q_align.setFromTwoVectors(gravity_dir, target_gravity);
    M3D R_align = q_align.toRotationMatrix();

    // 应用到初始旋转
    init_state.rot = Sophus::SO3(R_align);
